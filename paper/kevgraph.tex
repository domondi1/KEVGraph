...
Evaluated on 20 real-world open-source repositories (465 dependency
graphs, 911 vulnerabilities, 5 KEV-listed), \kevgraph's ILP planner
...
Both planners require only \textbf{427} upgrade actions to cover all 887
reachable vulnerabilities---13.3\% fewer than the random mean of 492.4.
...
We collected 20 open-source GitHub repositories with
...
\begin{table}[t]
\centering
\caption{Corpus of 20 open-source repositories}
\label{tab:repos}
\footnotesize
\begin{tabular}{ll}
\toprule
...
\begin{itemize}
  ...
  \item \textbf{911} unique vulnerability records from OSV,
        \textbf{743} of which have a non-zero CVSS score.
  \item \textbf{5} KEV-listed vulnerabilities affecting
        \textit{electron} (2), \textit{jquery} (1),
        \textit{puppeteer} (1), and \textit{vite} (1).
  \item \textbf{2,287} per-version candidate fixes covering
        \textbf{427} unique fixable packages.
  \item \textbf{887} coverable vulnerabilities
        (24 vulns have no known fix and are excluded from planning).
  \item \textbf{604} of 887 coverable vulnerabilities (68.1\%) appear
...
\begin{description}
  ...
  \item[\textbf{\#actions}] Total upgrade actions required to cover all
        887 reachable vulnerabilities.
...
\midrule
KEVGraph (ILP)     & 0.0248 & 0.0609 & \textbf{0.9972} & \textbf{1}  & \textbf{427} & 887 & 0.074 \\
KEVGraph (Greedy)  & 0.0248 & \textbf{0.0913} & 0.9105 & \textbf{1}  & \textbf{427} & 887 & 0.106 \\
EPSS-first         & 0.0248 & 0.0519 & 0.9869 & \textbf{1}  & 428 & 887 & 0.100 \\
CVSS-first         & 0.0011 & 0.0158 & 0.6392 & 18 & 429 & 887 & 0.084 \\
Random (seed=0)    & 0.0011 & 0.0124 & 0.6750 & 8  & 488 & 887 & 0.109 \\
Dependabot         & 0.0011 & 0.0068 & 0.5809 & 19 & 430 & 887 & 0.136 \\
\bottomrule
\end{tabular}
\end{table*}
...
converge to 100\% coverage at step 427.}
\label{fig:coverage}
\end{figure}
...
\caption{Total upgrade actions required per plan.  Both \kevgraph
planners require 427 actions (optimal), while random ordering requires
488 actions (seed=0) and averages 492.4 actions across 30 seeds.  The
...
\begin{itemize}
  ...
  \item \textbf{Both planners minimise action count.}  Both achieve 427
        upgrade actions---the ILP's proven optimum---versus 429--430
        for non-random baselines and 492.4 on average for random.
        Notably, 427 equals the number of distinct fixable packages in
        the corpus, confirming that the ILP achieves the theoretical
...
\midrule
\Tone   & 0.0021 & 0.0011 & 0.0067 & 0.0248 \\
\Tfive  & 0.0112 & 0.0056 & 0.0238 & 0.0609 \\
\aucc   & 0.7038 & 0.5788 & 0.8898 & 0.9972 \\
\#actions & 492.4 & 483.9 & 504.1  & 427    \\
\bottomrule
\end{tabular}
\end{table}
...
of 0.890, providing statistical evidence that the ILP's KEV
prioritisation is not achievable by random ordering alone.
For \#actions, the ILP requires 427 vs.\ a random mean of 492.4
(95\% CI $[483.9, 504.1]$), a reduction of 65.4 actions (13.3\% fewer).
...
\textbf{Certificate.}
The corpus-level remediation certificate records all 887 (fix, vuln)
...
\textbf{Corpus size.}  The current evaluation covers 20 repositories.
While the selected repositories span diverse domains and contain over
900 vulnerabilities, generalisability to the broader npm ecosystem
requires evaluation at larger scale.  We leave scale-up to future work.

\textbf{KEV sparsity.}  Only 5 of 911 vulnerabilities (0.55\%) are
KEV-listed in this corpus.  This sparsity means that KEV-first ordering
...
Evaluated on 20 real-world repositories with 911 vulnerabilities
(5 KEV-listed), our ILP planner achieves \aucc $= 0.997$ versus a
random-baseline mean of $0.704$ (bootstrap 95\% CI
$[0.579,\,0.890]$, $n=30$), ranks the first KEV vulnerability at
plan step~1, and requires only 427 upgrade actions (13.3\% fewer
than the random mean of 492.4).  CVSS-first and Dependabot-style
...
\end{verbatim}
Expected key values: ILP \aucc\,$=0.9972$, ILP \#actions\,$=427$,
random \aucc\ mean\,$=0.704$, random \#actions mean\,$=492.4$.
