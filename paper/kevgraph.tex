...
corpus.
Figure~\ref{fig:kev_heatmap} shows the step at which each KEV vulnerability
is first covered under each plan---the clearest illustration of the ordering
gap.
...
upgrade actions.
Figure~\ref{fig:action_eff} compares plan cardinality with the random CI.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../data/plots/action_efficiency.pdf}
\caption{Total upgrade actions required to cover all 937 coverable
vulnerabilities, per plan (npm corpus).
The error bar for the random baseline spans the empirical 95\% CI
$[481.9,\,506.3]$ ($n=30$ seeds).
Both \kevgraph planners achieve 417 actions---the ILP-proven minimum---
15.9\% fewer than the random mean of 495.4.
CVSS-first and EPSS-first also achieve near-optimal cardinality (419 and 417,
respectively) because all methods use the same set-cover fix generation;
the difference is \emph{ordering}, not cardinality.}
\label{fig:action_eff}
\end{figure}
