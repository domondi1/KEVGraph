...
the OSV vulnerability database and the CISA Known Exploited Vulnerability
(KEV) catalogue, (3) generates per-installed-version candidate upgrade
actions, and (4) produces a minimum-cardinality remediation plan ordered
to eliminate KEV exposures as early as possible.
...
achieves an \aucc score of \textbf{0.997} versus \textbf{0.704} for
random (95\% CI $[0.579, 0.890]$, $n=30$ seeds).
Both KEVGraph planners rank the first KEV vulnerability at step~1,
while CVSS-first defers it to step~18 and Dependabot to step~19.
Both planners require only \textbf{427} upgrade actions to cover all
887 reachable vulnerabilities, compared with 492.4 on average for
random ordering.
\end{abstract}
...
that are actively exploited in the wild.  A KEV-listed vulnerability
in a production dependency represents concrete, measurable exploitation
risk.  Prioritizing KEV elimination over other severity-based orderings
is directly actionable under CISA's Binding Operational Directive
22-01.
...
\begin{itemize}
  ...
  \item An exact ILP formulation of minimum-set-cover remediation.
...
\begin{itemize}
  ...
  \item \textbf{2,287} per-version candidate fixes across
        \textbf{427} unique packages.
...
\begin{itemize}
  \item \textbf{KEV-first ordering is decisive.}  Both \kevgraph
        ...
        first action is \texttt{upgrade:electron} which covers 2 of the
        5 KEV vulnerabilities).  CVSS-first defers the first KEV fix to
        step~18 and Dependabot to step~19.  This 18-step delay
        represents a window of unnecessary KEV exposure.
...
seeds and computed 95\% percentile confidence intervals via bootstrap
sampling.  Table~\ref{tab:random_ci} reports the results.
...
For \aucc, the ILP value of 0.997 is above the 95\% CI upper bound
of 0.890, confirming statistically that the ILP's KEV prioritisation
is not achievable by random ordering alone.  For \#actions, the ILP
requires 427 vs.\ a random mean of 492.4 (95\% CI $[483.9, 504.1]$),
a reduction of 65.4 actions (13.3\% fewer).
...
\subsection{Case Study: FredrikNoren/ungit}
\label{sec:case}

To illustrate \kevgraph's practical output, we examine one repository
from the corpus: \textbf{FredrikNoren/ungit}, a Git web interface
written in Node.js with 853 dependency nodes in its lockfile graph.

\textbf{Vulnerability landscape.}
The OSV join identified \textbf{180 unique vulnerabilities} across
packages in this repository's dependency graph.  Of these, \textbf{4
are KEV-listed}:
...
\begin{itemize}
  ...
  \item \texttt{GHSA-jpcq-cgw6-v4j6} (jquery):
        XSS vulnerability, CVSS 6.9
  \item \texttt{GHSA-c2gp-86p4-5935} (puppeteer):
        use-after-free, CVSS 6.5
\end{itemize}

\textbf{Greedy plan.}
The greedy planner's first action for this repository is:
\[
  \texttt{upgrade:electron@1.6.11} \to \texttt{@35.7.5}
\]
This single upgrade covers \textbf{22 vulnerabilities}, including both
KEV-listed electron vulns (CVSS 8.8 each), immediately eliminating the
two highest-severity actively-exploited exposures in the repository.

The ILP planner selects the same fix set (same total action count) and
also ranks the electron upgrade first, achieving \aucc $= 0.997$
across the corpus.
...
upgrades.  An organisation applying the Dependabot ordering spends
engineering time on lower-risk packages before addressing actively
exploited ones.

\textbf{Certificate.}
The remediation certificate for this repository records all (fix,
vuln) edges in the plan, enabling automated verification in under
200\,ms that all 180 vulnerabilities are covered by the plan.
...
\textbf{Set-cover-based security patching.}
Bauer et al.~\cite{bauer10} formulate patch scheduling as an
optimisation problem, but without KEV signals.  Our work introduces
KEV-aware ordering on top of the set-cover formulation.

\textbf{Automated dependency updates.}
Dependabot~\cite{dependabot}, Renovate~\cite{renovate}, and
WhiteSource Bolt provide automated pull requests for dependency
upgrades ordered by severity bucket.  \kevgraph differs by using
KEV membership as the primary ordering signal and by minimising
total upgrade count via set-cover optimisation.

\textbf{EPSS and KEV correlation.}
Spring et al.~\cite{spring21} show that CVSS score is a poor predictor
of exploitation.  Our results confirm this in the npm context:
CVSS-first defers KEV fixes to step~18 while EPSS-first (correlated
with KEV membership in our dataset) achieves step~1.

% ============================================================
\section{Conclusion}
\label{sec:conclusion}

We presented \kevgraph, an end-to-end pipeline for priority-ordered
vulnerability remediation in npm dependency ecosystems.  By framing
remediation as a KEV-aware set-cover problem, \kevgraph minimises the
number of required upgrade actions while ensuring that actively
exploited (KEV-listed) vulnerabilities are resolved as early as
possible in the remediation sequence.

Evaluated on 20 real-world repositories with 911 vulnerabilities (5
KEV-listed), our ILP planner achieves \aucc $= 0.997$ versus
$0.704$ mean for random ordering (95\% CI $[0.579, 0.890]$, $n=30$),
ranks the first KEV vulnerability at plan step~1, and requires only
427 upgrade actions (13.3\% fewer than the random mean of 492.4).
CVSS-first and Dependabot-style ordering---the current industry
practice---defer the first KEV fix to steps 18 and 19, respectively.
...
\bibitem{epss}
J.~Spring, E.~Hatleback, A.~Householder, A.~Manion, and D.~Shick,
``Prioritizing Vulnerability Response: A Stakeholder-Specific Vulnerability
Categorization,''
Carnegie Mellon University SEI Technical Report,
2021.
...
\bibitem{bauer10}
L.~Bauer, S.~Garriss, and M.~Reiter,
``Constraining Attacker Capabilities Through Обозначение,''
\emph{IEEE Transactions on Dependable and Secure Computing},
2010.
