...
of \textbf{0.688} (empirical 95th-percentile interval $[0.541,\,0.902]$, $n=30$ seeds),
...
coverable vulnerabilities---13.7\% fewer than the random mean of 504.1.
...
\begin{enumerate}
  ...
  \item \textbf{Evaluate} -- compute all metrics for every plan;
        compute empirical 95th-percentile intervals for the random ensemble; write
...
The ILP is provably optimal in remediation cardinality.  After the
optimal set is selected, fixes are ordered using the same KEV-aware
priority function as the greedy planner (post-processing step);
joint optimality of cardinality and KEV-early ordering is not
...
\begin{description}
  ...
  \item[\textbf{\#actions}] Total upgrade actions required to cover all 912 coverable vulnerabilities.
...
\begin{itemize}
  ...
  \item \textbf{ILP dominates on \aucc.}  The ILP achieves
        ...
        greedy planner achieves 0.9090 due to occasional non-KEV fixes
  ...
  \item \textbf{CVSS-first performs poorly on KEV timing.}
        CVSS-first's kev\_first\_rank $= 18$ and \aucc $= 0.638$
...
\begin{table}[t]
\centering
\caption{Empirical 95th-percentile interval for random baseline ($n=30$ seeds).
...
(95\% CI $[494.2, 515.5]$), a reduction of 69.1 actions (13.7\% fewer).

The random baseline's \aucc mean of 0.688 (CI $[0.541, 0.902]$) is
...
close to 0.90, underscoring that the \emph{worst-case} exposure
under random ordering is substantially worse (0.541).
...
The Dependabot-style baseline's alphabetical-within-severity-bucket ordering delays the
...
CVSS-first's kev\_first\_rank $= 18$ and \aucc $= 0.638$ demonstrate
...
\aucc ($0.909$ vs.\ $0.997$).  Practitioners who need to maximise
...
treating KEV membership as the primary ordering priority within a
cardinality-optimal remediation set.  The accompanying certificate
...
exploitation-aware, cardinality-optimal vulnerability remediation in npm
...
random-baseline mean of $0.688$ (empirical 95th-percentile interval
$[0.541,\,0.902]$, $n=30$ seeds), ranks the first KEV vulnerability at
...
independent verification that the plan covers all coverable
