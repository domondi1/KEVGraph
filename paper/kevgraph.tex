...
corpus.
Figure~\ref{fig:cvss_kev} explains the root cause of CVSS-first's failure:
186 non-KEV vulnerabilities have CVSS~$>$~8, crowding out the lower-CVSS
KEV-listed packages (vite: 5.3, puppeteer: 6.5, jquery: 6.9) in severity
ordering.
Figure~\ref{fig:kev_heatmap} shows the resulting step at which each KEV
vulnerability is first covered under each plan---the clearest illustration
of the ordering gap.
Figure~\ref{fig:aucc} shows \aucc scores with the random 95\% CI band.
Figure~\ref{fig:kev_curve} shows KEV-specific coverage in the first 30
upgrade actions.
...
\caption{CVSS base score vs.\ EPSS 30-day exploitation probability for all
1,046 npm vulnerabilities (grey dots); 5 KEV-listed vulnerabilities highlighted
as diamonds.
\textbf{CVSS does not predict active exploitation:} 186 non-KEV vulnerabilities
have CVSS~$>$~8, outranking vite (5.3), puppeteer (6.5), and jquery (6.9) in
CVSS-first ordering---mechanistically explaining the 17-step KEV exposure gap.
\textbf{EPSS correlates better but has a critical outlier:} four of five KEV
vulnerabilities have EPSS~$>$~0.35; however, electron/libvpx (EPSS~$=$~0.048)
is not identifiable from EPSS alone, motivating the use of the authoritative
KEV catalogue as the primary ordering signal.}
\label{fig:cvss_kev}
\end{figure}
...
\caption{Step at which each KEV vulnerability is first covered, per plan
(npm corpus; green = early, red = late; cell value = plan step number).
\kevgraph (ILP) covers all five KEV vulnerabilities by step~4.
CVSS-first and Dependabot do not cover the first KEV until step~18,
and do not reach full KEV coverage until steps 251 and 314, respectively.
This 17-step initial exposure gap represents remediation actions spent
on lower-risk packages while actively-exploited vulnerabilities remain open.}
...
\caption{\aucc scores for all plans on the npm corpus.
The shaded band is the empirical 95\% CI of the random baseline
($[0.519,\,0.831]$, $n=30$ seeds).
\kevgraph (ILP) achieves \aucc $= 0.997$---0.166 above the CI upper bound---
providing statistical evidence that the observed KEV-prioritisation
advantage is not achievable by chance.
Dependabot achieves the lowest \aucc ($0.589$), reflecting its
severity-bucket ordering that is insensitive to KEV membership.}
\label{fig:aucc}
\end{figure}
...
\caption{KEV-specific coverage in the first 30 upgrade actions (npm corpus,
5 KEV-listed vulnerabilities).
Unlike total-coverage curves (all plans converge to 100\% at step~417),
this view isolates the ordering objective.
\kevgraph (ILP) covers all 5 KEV vulnerabilities by step~4;
\kevgraph (Greedy) covers 3 by step~4 but does not reach full KEV coverage
until step~155 (puppeteer).
CVSS-first and Dependabot remain at zero KEV coverage through the entire
30-step window shown.}
\label{fig:kev_curve}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Both planners minimise upgrade count.}  ILP and Greedy
        both require 417 upgrade actions---the ILP-proven minimum (Table~\ref{tab:random_ci})---
        15.9\% fewer than the random mean of 495.4.
        All non-random baselines also achieve near-optimal cardinality (417--421),
        confirming that the key differentiator between plans is \emph{ordering},
        not total action count.

  \item \textbf{CVSS-first fails because CVSS does not predict exploitation.}
        As shown in Figure~\ref{fig:cvss_kev}, 186 non-KEV vulnerabilities have
        CVSS~$>$~8, all outranking vite (CVSS~5.3), puppeteer (6.5), and
        jquery (6.9) in CVSS-first ordering.
        CVSS-first's kev\_first\_rank $= 18$ and \aucc $= 0.708$ are a direct
        consequence: the system spends 17 upgrade actions on theoretically severe
        but unexploited packages before addressing any actively-exploited one.
\end{itemize}
...
\caption{Cross-ecosystem KEV-prioritisation advantage (forest plot).
Each row shows the \aucc of \kevgraph (ILP, diamond) and EPSS-first
(triangle) against the random baseline 95\% CI (shaded bar, $n=30$ seeds).
In all three ecosystems, the ILP result lies above the CI upper bound.
Maven shows the largest absolute gap ($0.988 - 0.693 = +0.295$ above CI;
gap to random mean $= 0.502$), consistent with its higher KEV density (2.37\%).
PyPI achieves perfect ILP \aucc $= 1.000$ with a wide random CI
($[0.232,\,0.979]$), confirming meaningful ordering advantage even
when KEV density is low.}
\label{fig:ecosystem}
\end{figure}
...
\caption{Planning runtime vs corpus size (npm, log scale).
\kevgraph (ILP, CBC solver) solves in under 130\,ms across all corpus
sizes tested (50--924 repositories, 2,366--2,741 candidate fixes),
well below a 200\,ms automation budget.
Greedy time grows sub-linearly (up to 4.4\,s at full scale) due to
the iterative set-cover selection loop.
Both planners are practical for CI/CD integration.}
...
\textbf{EPSS-first is competitive but not complete.}
The EPSS-first baseline achieves kev\_first\_rank $= 1$ and \aucc $= 0.989$,
close to the ILP's 0.997.
Figure~\ref{fig:cvss_kev} shows why EPSS performs well: four of five KEV
vulnerabilities have EPSS~$>$~0.35, far above the npm corpus median.
However, electron/libvpx has EPSS~$=$~0.048---a KEV vulnerability that EPSS
does not surface.  The residual \aucc gap (0.009) is attributable in part
to this outlier: a vulnerability confirmed actively exploited by CISA but
not yet reflected in FIRST's probability model at the time of evaluation.
EPSS scores are probabilistic and lag behind KEV list additions by the
FIRST API update cadence; KEV membership is a deterministic, authoritative
signal.  The ILP provides a rigorous optimality guarantee on cardinality
that EPSS-first cannot claim.

\textbf{CVSS is a poor proxy for exploitability.}
Figure~\ref{fig:cvss_kev} makes this concrete: 186 non-KEV vulnerabilities
in our npm corpus have CVSS~$>$~8.0, including 118 with CVSS~$>$~9.0.
These outrank KEV-listed packages such as vite (CVSS~5.3) and
puppeteer (CVSS~6.5) in severity ordering, causing CVSS-first to spend
17 upgrade actions on unexploited packages before reaching the first
actively-exploited vulnerability.
\aucc~$= 0.708$ and kev\_first\_rank~$= 18$ follow directly.
This finding aligns with prior empirical studies of KEV vs.\ CVSS
correlation~\cite{spring21} and extends them to the npm ecosystem.
